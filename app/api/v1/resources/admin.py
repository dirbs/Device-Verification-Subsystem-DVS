...
from ..assets.responses import responses, messages, mime_types
            ...
            if tac.isdigit():  # TAC format validation
                ...
                if full_status['gsma']:  # TAC verification
                    ...
                    if seen_with == 1:
                        ...
                        return Response(json.dumps(response), status=status, mimetype=mime_types.get('json'))

                    return Response(json.dumps(response), status=responses.get('ok'), mimetype=mime_types.get('json'))
                else:
                    return custom_response("IMEI not found", responses.get('not_found'), mimetype=mime_types.get('json'))
            else:
                return custom_response("Bad TAC format", responses.get('bad_request'), mimetype=mime_types.get('json'))
